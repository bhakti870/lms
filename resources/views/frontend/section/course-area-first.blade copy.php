    ...
    <div class="container">
        ...
        <ul class="nav nav-tabs generic-tab justify-content-center pb-4" id="myTab" role="tablist">
            ...
            @foreach ($categories as $index => $item)
                <li class="nav-item">
                    <button class="nav-link {{ $index == 0 ? 'active' : '' }}" id="{{ $item->slug }}-tab" data-bs-toggle="tab"
                        data-bs-target="#{{ $item->slug }}" type="button" role="tab" aria-controls="{{ $item->slug }}"
                        aria-selected="{{ $index == 0 ? 'true' : 'false' }}">{{ $item->name }}</button>
                </li>
            @endforeach
